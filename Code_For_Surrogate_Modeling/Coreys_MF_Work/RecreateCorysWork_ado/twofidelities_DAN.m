function [] = twofidelities_DAN()
    gb.x0 = .55;
    gb.lb = [-2,-2];
    gb.ub= [2,2];
    algorithm = 'interior-point'; %testpoints = linspace(0,1.1,1000)';
    highfidelity = @(x)hfm(x);
    lowfidelity = @(x)lfm(x);
...
figure , hold on
plot(testpoints, highfidelity(testpoints),'b')
plot(testpoints,lowfidelity(testpoints),'r')
plot(testpoints, correctedlf(testpoints,@(x)lowfidelity(x),wr,model),'k','linewidth',2)
scatter(gb.x, gb.low,100)
scatter(gb.x, gb.high,100)
scatter(gb.xOpt, gb.lowOpt,100,'fill','k')
scatter(gb.xOpt, gb.highOpt,100,'fill','k')
...
function [f,df] = hfm(x) %this 
       f = ((6.*x-2).^2).*sin(12.*x-4);
    df =  (12.*(6*x-2).^2).*cos(4-12.*x)-12.*(6.*x-2).*sin(4-12.*x);
